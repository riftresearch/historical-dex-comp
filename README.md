...
bun run index.ts ingest-loop backfill_older --providers lifi,relay,thorchain,chainflip,garden,nearintents,kyberswap --stop-at-oldest 2026-01-01T00:00:00Z
PROVIDERS=lifi,relay,thorchain,chainflip,garden,nearintents,kyberswap MODE=backfill_older STOP_AT_OLDEST=2026-01-01T00:00:00Z docker compose up -d ingester
...
START_AT=$(date -u -d '7 days ago' +%Y-%m-%dT%H:%M:%SZ); END_AT=$(date -u +%Y-%m-%dT%H:%M:%SZ); for p in lifi relay thorchain chainflip garden nearintents kyberswap; do bun run implementation-shortfall -- --provider "$p" --source-chain eip155:1 --source-token ETH --destination-chain bitcoin:mainnet --destination-token BTC --start-at "$START_AT" --end-at "$END_AT" --min-notional-usd 1000 --max-notional-usd 10000 --stablecoin-peg-mode usd_1; done
bun run typecheck
```
...
- Current ingestion implementation is wired for `lifi`, `relay`, `thorchain`, `chainflip`, `garden`, `nearintents`, and `kyberswap`.
...
- `integrity` samples provider API records and checks they are already persisted in provider DuckDB (`swaps_core`) with hit-rate + Wilson lower-bound stats.
- `integrity` currently has no API sampler implementation for `kyberswap`; it reports this provider as unsupported in audit output.
...
- `regen-frontend-precomputed` runs the full frontend precompute matrix in one command (`lifi,relay,thorchain,chainflip,garden,nearintents,kyberswap` x `USDC,USDT,ETH` on `eip155:1 -> bitcoin:mainnet`, default last 30 days, `--stablecoin-peg-mode usd_1`).
- If `kyberswap`, `lifi`, and/or `relay` are included in `regen-frontend-precomputed`, it also precomputes `USDC/USDT/ETH/WETH -> CBBTC` paths on `eip155:1` for those providers.
...
- The frontend reads `/api/shortfall-distribution` and renders per-provider shortfall histogram charts for fixed 30-day windows across `USDC/USDT/ETH` (`eip155:1`) to `BTC` (`bitcoin:mainnet`) paths plus KyberSwap-only `USDC/USDT/ETH/WETH -> CBBTC` (`eip155:1`) paths.
...
- KyberSwap ingestion requires `KYBERSWAP_RPC_URL` and optionally `KYBERSWAP_META_AGGREGATOR` (defaults to `0x6131b5fae19ea4f9d964eac0408e4408b66337b5`).
